    ...
    async transaction(
        ...
        switch (tipo) {
            ...
            case Tipo_Transaccion.Guardar_Con_Parametros:
                try {
                    await queryRunner.manager.save(entidad, { [campo_actualizar]: datos_entidad });
                    await queryRunner.commitTransaction();
                    return 'Éxito';
                } catch (error) {
                    await queryRunner.rollbackTransaction();
            ...
            case Tipo_Transaccion.Actualizar_Con_Parametros:
                try {
                    let resultado = await queryRunner.manager.update(entidad, campo_id, { [campo_actualizar]: datos_entidad });
                    await queryRunner.commitTransaction();
                    return {
                        mensaje: 'Éxito',
                        resultado: resultado
